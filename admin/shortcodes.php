	...
	function apfl_pp_shortcodes_reference_callback() {
		...
		$sections = array(
			array(
				...
				array('city', __('one or comma-separated', 'appfolio-listings-custom'), __('Pre-filters by Appfolio city on first load (same as City dropdown). Only when the visitor has not submitted the filter form. Values must match Appfolio option text exactly (e.g. Templeton). If no rentals exist in the requested city, the no-results message is shown instead of falling back to all listings.', 'appfolio-listings-custom')),
				array('all_listings_url', __('URL', 'appfolio-listings-custom'), __('Optional. Used together with city. When no rentals are found in the requested city, displays "It looks like we don\'t have any available rentals right now in [City]" with a "View All Available Rentals" button linking to this URL. If omitted, the plain city no-results text is shown instead.', 'appfolio-listings-custom')),
				array('empty_message', __('HTML or text', 'appfolio-listings-custom'), __('Optional. When the grid has no listing items, show this instead of the default no-results message. Basic HTML is allowed (same rules as post content). Ignored when listings are found. Overrides the all_listings_url city message. Developers can override the final HTML with the apfl_listings_no_results_html filter.', 'appfolio-listings-custom')),
				),
			),
			array(
